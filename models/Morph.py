...
class LocalSim(nn.Module):
    ...
    def forward(self, x_in, y_in):
        ...
        affinity = affinity.view(-1, d, h, w, self.channel).permute(0, 4, 1, 2, 3).contiguous()
        return affinity
class GlobalSim(nn.Module):
    ...
    def forward(self, x_in, y_in):
        b, c, d, h, w = x_in.shape
        x = x_in.flatten(start_dim=2).permute(0, 2, 1) # b, n, c
        y = y_in.flatten(start_dim=2).permute(0, 2, 1) # b, n, c
        x = self.normx(x)
        y = self.normy(y)
        affinity = get_aff(x, y)
        affinity = affinity.view(b, d, h, w, -1).permute(0, 4, 1, 2, 3).contiguous()
        return affinity
...
class Morph(nn.Module):
    def __init__(self, in_channel=16, channel_num=16):
        ...
        self.encoder = Encoder(channel_num=in_channel)

        self.conv_1 = DualConvBlock(in_channel * 1 * 2 + channel_num * 1, channel_num * 1)
        self.conv_2 = DualConvBlock(in_channel * 2 * 2 + channel_num * 2 + 8, channel_num * 2)
        self.conv_3 = DualConvBlock(in_channel * 4 * 2 + channel_num * 4 + 8, channel_num * 4)
        self.conv_4 = DualConvBlock(in_channel * 8 * 2 + channel_num * 8 + 8, channel_num * 8)
        self.conv_5 = DualConvBlock(in_channel * 16 * 2 + 1680, channel_num * 16)

        self.corr_2 = LocalSim(in_channel * 2)
        self.corr_3 = LocalSim(in_channel * 4)
        self.corr_4 = LocalSim(in_channel * 8)
        self.corr_5 = GlobalSim(in_channel * 16)
